...
ww_trans.pca <- prcomp(ww_trans, scale=TRUE)
...
```
The correlation r value between PC1 and quality is 0.398, which is not better than correlation between quality and alcohol alone (0.436). So, using all variables into PCA does not improve in explaining quality.
...
```
![Alt text](/3d_pca.JPG)
The 3D-plot of PCA shows better separation of the classes.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
qgraph.pca(ww_trans[, c(1,2,3,4,10,12)], factors=2, rotation="varimax")
```
```{r echo=FALSE, fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
...
```
Well, it dosen't help much. The correlation r value is 0.330 but the classes become spread out compared to all variables PCA.
...
Again, the seperation of classes becomes worse with the selected variables pca.
