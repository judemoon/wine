...
qual.alco2 <- lm(quality ~ alcohol, data = ww[ww$quality>4, ])
...
As I mentioned previously, the linear model with combination of variables help increase a little bit of r-squared value, but not by much. And the prediction power is lower than 45% where predicting all wines as median quality (6) irrespect to the physicochemical properties. It indicates that a linear model probably is not the best fit for this dataset. As the distribution of sugar contents (bimodality) and free&total SO2 and acetic acid (threshold effects) hinted, decision tree could be a good candidate, where a model predicts the value of a target variable by learning simple decision rules inferred from the data features. 
...
### Description Three
PCA biplot was contructed by 1st dimiention scores on the x-axis and 2nd dimention scores on the y-axis and the points represent individual observations in the dataset, which were color-encoded by quality groups (<6, 6, and >6). The arrows represent correlation of each variables with the two dimention scores: the direction of arrow represents either positive or negative correlation, and the length represents magnitude of correlation coefficient. For example, the arrow of alcohol heads for the positive direction of 1st dimention score with relativel high length. This means that alcohol is strongly and positively correlated with Dim1.

There was a general trend from low to high quality as shifting from left to right, although the separation of groups was not quite clear. One of interesting interactions was that some of physicochemical properties were very dependant on each other. Increase in density of wines was influenced largely by decrease in alcohol and increase in most solutes such as sugar, NaCl, and SO2. In addition, increase in pH (decrease in acidity) was affected by decrease in tartaric acid and citric acid. This plot helped understand the relationship among the physicochemical properties rather than suggest a prediction for sensory quality.

One of my speculations on these trends with wine quality is that too much amount of any solutes might bring unpleasant smell and taste to wine, which was suggested by the arrows of all solutes were heading to the left where low quality wines more likely present around these coordinant. The high amount of solutes could be resulted from too early or too late harvesting time of wine product. If it is too early, a large amount of sugar remains in the wine, resulting in being too sweat. If it is too late, alcohol decompose to acetic acid, resulting in being too sour. 

Overall, I have learned that predicting the quality of wine is very complicated and hard to accomplished solely based on physicochemical properties given. The limitation of these properties is only examing chemicals in the liquid stage. So, any gas form of chemicals such as aroma can be complementary information.   
------

# 4. Reflection
One of my frustrations as a newbie to R was tuning plots one by one, like adjusting bin size, font, color, label, etc. Although I knew that all the plots did not need to be perfect at the exploring stage, tuning steps to increase aesthetic factors in plots took me a large chunk of time. To figure out how to change the visual factors I had to search examples and documentation of each library, becuase a lot of libraries do not share same functions and arguments. Despite the frustrations, I think R offers advantages such as flexibility and a lot of accessible libraries. Before this project, I have been using python for exploring datasets (e.g. titanic dataset). Compared to the python jupyter notebook, I have felt that the syntex of R is easiler and less restricted, and R provides a lot better graphs and more complex statistics with a simple line of code. But I had hard time to make functions or loops to avoid repeating codes. Unlike python, it lacks iterating functionality. I prefer to copy and paste codes rather than writing function, particularily when I try to draw chart. And another drawback, I felt, is that R runs slower than python and sometimes RStuio stopped running while I was working on this project. 

I have tried 3 different types of PCA functions: prcomp, and princomp, and psych.principal. How to get a summary about PCA matrix data was quite similar by using the tree methods, but for plotting PCA, each method can be implemented in a different plotting library. They were not exchangeable sometimes. What I wanted to do for the PCA biplot was to have variable vectors and to color data points by a group. I have tried many different arguments and attributes but I found a library to visualize plots specially for PCA with very minimum lines of codes, instead of extracing PCA scores into dataframe and making plots using ggplot.

I think the next step for this project is performing machine learning algorithms. As I mentioned previously, decision tree can be one of candidate classifiers or regressions. 
------
