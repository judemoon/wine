...
```{r echo=FALSE, message=FALSE, warning=FALSE, outlier}
...
# Create a function to generate a boxplot with y-axis as quality.con and x-axis as input column
plot2 <- function(col, quality, df, lab) {
  qplot(x= quality, y= col, data = df, geom = 'boxplot', ylab = lab)+ 
  geom_jitter(alpha = 1/6, color = 'royalblue')+
    coord_flip() # make horizontal boxplot
}

# Iterate every column to generate boxplot from ww_trans dateframe
for (i in 1:11) {
  print(plot2(ww_trans[ , i], ww_trans$quality.con, ww_trans, colnames(ww_trans)[i]))
}
```
...
aov_table <- function (col) {
  col.aov <- aov(col ~ ww_trans$quality.con)
  summary(col.aov)
}
...
```{r echo=FALSE, warning=FALSE, Bivariate_Plots_density_by_quality}
...
for (i in 1:11) {
  print(plot3(ww_trans[ , i], ww_trans$quality.lib, ww_trans, colnames(ww_trans)[i]))
}
```
