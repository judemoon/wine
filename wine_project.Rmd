...
Variable citric.acid has 19 zeros. I will assume that they are true zeros.
...
Plot1 <- function(alcohol, group) {
  a <- qplot(x = alcohol, data = ww, binwidth = 0.1, color = I('black'), fill = group)+
  scale_x_continuous(limits = c(min(alcohol), max(alcohol)),
                     breaks = seq(8, max(alcohol), 1))
  b <- table(group)
  c <- ProportionTable(group)
  return(list(a, b, c))
}
...
The distribution is little bit skewed to the left with mostly (68%) being light-bodied (alcohol <= 11). But I don't think that it requires transformation because the skewness seems minor. 
...
There was a general trend from low to high quality as shifting from left to right, although the separation of groups was not quite clear. One of interesting interactions was that some of physicochemical properties were very dependent on each other. Increase in density of wines was influenced largely by decrease in alcohol and increase in most solutes such as sugar, NaCl, and SO2. In addition, increase in pH (decrease in acidity) was affected by decrease in tartaric acid and citric acid. This plot helped understand the relationship among the physicochemical properties rather than suggest a prediction for sensory quality.

One of my speculations on these trends with wine quality is that too much amount of any solutes might bring unpleasant smell and taste to wine, which was suggested by the arrows of all solute variables were heading to the left where low quality wines more likely present around left-side coordinate. The high amount of the solutes could be resulted from too early or too late harvesting time of wine product. If its harvesting is too early, a large amount of sugar remains in the wine, resulting in being too sweat. If it is too late, alcohol decompose to acetic acid, resulting in being too sour. Alternatively, the sensory quality evaluation might be influenced by bias of expert tasters, who have a role to guide sort of standards of wine quality to the wine industry and customers. I think this might lead that wine experts value well-balanced flavors rather than exotic and unique flavors of wine for a broader range of customers. As a result, the wine with extreme or unique range of solutes might be rated low.  
